...
local function change_subgroups(items, recipes, mod_handle)
  ...
  BioInd.show("Checking item", item.name)
  BioInd.show("Item has 5D-subgroup", item[subgroup] or "nil")
  ...
  BioInd.writeDebug("Must add recipe %s to subgroup %s", {recipe.name, item[subgroup]})
