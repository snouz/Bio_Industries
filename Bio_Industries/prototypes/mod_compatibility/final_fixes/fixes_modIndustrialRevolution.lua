...
------------------------------------------------------------------------------------
BioInd.debugging.writeDebug("Change vanilla rails?")

if recipes[BI.additional_recipes.BI_Rails.rail_wood.name] then
BioInd.debugging.writeDebug("Wooden rails are active")
  recipe = recipes["rail"]
  -- Wood beam
  old = items["wood-beam"]
  ...
  if old then
    if not new then
      ...
      BioInd.debugging.modified_msg("ingredient \"" .. item .. "\"", recipe, "Removed")
    else
      thxbob.lib.recipe.replace_ingredient(recipe.name, old.name, new.name)
      BioInd.debugging.modified_msg("ingredient \"" .. old.name .. "\"", recipe, "Replaced")
    end
  end
...
local map = {
  -- Technology prerequisite 'ir2-concrete-2' on 'artillery' is redundant as 'rocket-silo' already contains it in its prerequisite tree.
  -- Technology prerequisite 'ir2-concrete-2' on 'artillery' is redundant as 'military-4' already contains it in its prerequisite tree.  ["artillery"] = {"tank"},
  -- Technology prerequisite 'tank' on 'artillery' is redundant as 'military-4' already contains it in its prerequisite tree.
  ["artillery"] = {"ir2-concrete-2", "tank"},
  ...
  -- Technology prerequisite 'ir2-concrete-1' on 'bi-tech-electric-energy-super-accumulators' is redundant as 'electric-energy-accumulators' already contains it in its prerequisite tree.
  ["bi-tech-electric-energy-super-accumulators"] = {"electric-energy-distribution-2", "ir2-concrete-1"},

  -- Technology prerequisite 'ir2-concrete-1' on 'bi-tech-huge-substation' is redundant as 'electric-energy-distribution-2' already contains it in its prerequisite tree.
  ["bi-tech-huge-substation"] = {"ir2-concrete-1"},

  -- Technology prerequisite 'fluid-handling' on 'bi-tech-steamsolar-combination' is redundant as 'ir2-solar-energy-1' already contains it in its prerequisite tree.
  ["bi-tech-steamsolar-combination"] = {"fluid-handling"},
  ...
  -- Technology prerequisite 'ir2-concrete-1' on 'bi-tech-wooden-storage-3' is redundant as 'logistics-3' already contains it in its prerequisite tree.
  ["bi-tech-wooden-storage-3"] = {"ir2-concrete-1"},
  ...
  -- Technology prerequisite 'ir2-bronze-milestone' on 'ir2-scatterbot' is redundant as 'ir2-scattergun-turret' already contains it in its prerequisite tree.
  ["ir2-scatterbot"] = {"ir2-bronze-milestone"},
