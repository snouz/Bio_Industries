...
for k, v in ipairs({"electric-pole", "item", "recipe"}) do
  ...
  if prototype then
    ...
    --~ BioInd.show("Changing localization for %s \"%s\".", {v, prototype.name})
...
if recipe then
  ...
  --~ BioInd.writeDebug(Changed subgroup of recipe \"%s\".", {recipe.name})
...
if crusher then
  ...
  --~ BioInd.writeDebug("Changed crafting categories of \"%s\".", {crusher.name})
