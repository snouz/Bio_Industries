  ...
  --~ repeat
    ...
    --~ for i, item in ipairs(i_data) do
      ...
      --~ if not data.raw[item.type][item.name] then
        --~ BioInd.debugging.show("Removing non-existent item", item.name)
...
local function get_one_result_recipes(recipe)
  ...
  if type(recipe) == "table" and recipe.name then
    ...
    if not recipes[recipe.name] then
      BioInd.debugging.writeDebug("Recipe %s does not exist -- returning immediately!", {recipe.name})
    -- Check recipe
    else
      ...
      if not results or table_size(results) ~= 1 then
        BioInd.debugging.writeDebug("Ignoring recipe %s (has %s results)",
        {recipe.name, results and table_size(results) or 0})
      else
        local r, result = next(results)
        -- We can't stack or crate fluids!
        if result.type == "fluid" then
          BioInd.debugging.writeDebug("Ignoring recipe %s (result is a fluid!)", {recipe.name})
        -- We've struck gold!
        else
...
BioInd.debugging.writeDebug("Complete recipe list: %s", {recipe_list})
...
if deadlock then
BioInd.debugging.writeDebug("Must create item stacks!")
...
BioInd.debugging.show("subgroup", subgroup)
BioInd.debugging.show(subgroup, data.raw["item-subgroup"][subgroup])
      if not data.raw["item-subgroup"][subgroup] then
BioInd.debugging.writeDebug("Must create subgroup %s!", subgroup)
        BioInd.create_stuff({
          type = "item-subgroup",
          name = subgroup,
          group = group.name,
...
BioInd.debugging.writeDebug("item name: %s (%s)", {item.name, item.type})
      check_item = data.raw[item.type][item.name]
BioInd.debugging.writeDebug("check_item: %s\trecipe_list[%s]: %s", {check_item and check_item.name or "nil", item.name, recipe_list[item.name] and "true" or "false"})
      if check_item and recipe_list[item.name] then
        stack_name = "deadlock-stack-" .. item.name
BioInd.debugging.show("stack_name", stack_name)
        new_item = items[stack_name]
BioInd.debugging.show("new_item.name", new_item and new_item.name or "nil")
        -- Create new item if necessary
        if not new_item then
BioInd.debugging.writeDebug("Must create stack!")
        -- deadlock.add_stack(item_name, graphic_path, target_tech, icon_size, item_type, mipmap_levels)
          ...
          new_item = items[stack_name]
BioInd.debugging.show("Created new_item", new_item and new_item.name or "nil")
          items[stack_name].item_subgroup = "stacks-" .. tostring(check_item.item_subgroup or "no_name")
BioInd.debugging.show("subgroup of stack item", items[stack_name].item_subgroup)
          BioInd.debugging.created_msg(new_item)
        end
        ...
        BioInd.debugging.modified_msg("localization", new_item)

        -- Modify new recipes
        for r, r_type in ipairs({"stack", "unstack"}) do
          new_recipe = recipes["deadlock-stacks-" .. r_type .. "-" .. item.name]
BioInd.debugging.show("new_recipe.name", new_recipe and new_recipe.name or "nil")
          if new_recipe then
            -- Localization
            new_recipe.localised_name = {"recipe-name.deadlock-stacking-" .. r_type, loc_name}
            BioInd.debugging.modified_msg("localization", new_recipe)
...
--~ for i, item in pairs(items) do
  --~ if i:match("deadlock") then
    --~ BioInd.debugging.writeDebug("Item %s: %s", {i, item.localised_name}, "line")
  --~ end
--~ end
--~ for r, recipe in pairs(recipes) do
  --~ if r:match("deadlock") then
    --~ BioInd.debugging.writeDebug("Recipe %s: %s", {r, recipe.localised_name}, "line")
  --~ end
--~ end
...
if deadlock_crating then
BioInd.debugging.writeDebug("Must create crates!")

  for level, i_data in pairs(crated_items) do
    for i, item in ipairs(i_data) do
BioInd.debugging.writeDebug("%s: %s", {i, item and item.name or "nil"})
      -- Some things (rail-planner, ammo etc.) are considered items although they are
      -- of another prototype type (but the stack items are of type "item")!
      if items[item.name] and recipe_list[item.name] then
        crate_name = "deadlock-crate-" .. item.name
        new_item = items[crate_name]
BioInd.debugging.show("new_item", new_item and new_item.name or "nil")
        -- Create new item if necessary
        if not new_item then
...
BioInd.debugging.show("new_item", new_item and new_item.name or "nil")
        ...
        BioInd.debugging.show("localised name", {"item-name.deadlock-crate-item", items_per_crate, loc_name})
        BioInd.debugging.modified_msg("localization", new_item)
        ...
        for r, r_type in ipairs({"pack", "unpack"}) do
          ...
          if new_recipe then
            ...
            new_recipe.subgroup = item.order_crating and
                                    ...
                                    src_recipe.subgroup
            BioInd.debugging.modified_msg("subgroup", new_recipe)
...
--~ BioInd.debugging.writeDebug("Show deadlock crating items")
--~ for i, item in pairs(items) do
  --~ if i:match("deadlock") and i:match("crat") then
    --~ BioInd.debugging.writeDebug("Item %s: %s", {i, item.localised_name or "no localization"}, "line")
  --~ end
--~ end
--~ for r, recipe in pairs(recipes) do
  --~ if r:match("deadlock") and r:match("crat") then
    --~ BioInd.debugging.writeDebug("Recipe %s: %s", {r, recipe.localised_name or "no localization"}, "line")
  --~ end
--~ end
