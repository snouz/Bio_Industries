...
------------------------------------------------------------------------------------
BioInd.debugging.writeDebug("Make list of potential fuel items")
...
BioInd.debugging.writeDebug("Name of %s \"%s\" matches whitelisted pattern %s", {item_type, item_name, pattern})
          check = true
          break
        end
      end
      ...
      if not check then
        ...
        for p, pattern in pairs(blacklist_patterns) do
          -- Match found!
          if item_name:match(pattern) then
BioInd.debugging.writeDebug("Name of %s \"%s\" matches blacklisted pattern \"%s\"", {item_type, item_name, pattern})
            black_check = true
    ...
    if check then
BioInd.debugging.writeDebug("Adding %s \"%s\" to potential_fuel_items", {item_type, item_name})
    ...
    -- Ignore this item!
      ...
      cnt = cnt + 1
BioInd.debugging.writeDebug("Ignoring %s \"%s\" (%s)", {
  item_type,
  item_name,
...
--~ BioInd.debugging.show("potential_fuel_items", potential_fuel_items)
--~ BioInd.debugging.show("fuel_items", fuel_items)

--~ BioInd.debugging.show("Number of potential_fuel_items", table_size(potential_fuel_items))
--~ BioInd.debugging.show("Number of fuel_items", table_size(fuel_items))
--~ BioInd.debugging.show("Number of ignored items", cnt)
--~ BioInd.debugging.show("potential_fuel_items", potential_fuel_items)
...
--~ for k, v in pairs(potential_fuel_items) do
  --~ test[k] = v.type
--~ end
--~ BioInd.debugging.show("Potential fuel items", test)
...
BioInd.debugging.show("Checking recipe", r)
...
BioInd.debugging.writeDebug("Recipe %s has results: %s", {recipe.name, results or "nil"}, "line")
  if results then
...
BioInd.debugging.writeDebug("Could result %s of recipe %s be a fuel item?", {r_name, recipe.name})

      -- Result is a potential fuel_item
      if potential_fuel_items[r_name] then
BioInd.debugging.writeDebug("Yes: %s", {results}, "line")
        ingredients = BI_Functions.lib.get_recipe_ingredients(recipe)
BioInd.debugging.writeDebug("Recipe %s has ingredients: %s", {recipe.name, ingredients or "nil"}, "line")
        -- Check recipe ingredients
        for i_name, ingredient in pairs(ingredients or {}) do
BioInd.debugging.show("Checking ingredient", i_name)
...
BioInd.debugging.writeDebug("Added %s \"%s\" to list of ingredients: %s",
                  {ingredient.type, i_name, ingredient}, "line")
          end
          -- Add result to fuel_items if it requires any fuel_items
          if fuel_items[i_name] then
            fuel_items[r_name] = {}
BioInd.debugging.writeDebug("Added %s \"%s\" to fuel_items!", {base_fuel.type, r_name})
          end
        end
        -- Only add recipe data to the item if relevant ingredients are used
BioInd.debugging.show("final list", i_list)
        if table_size(i_list) > 0 then
          potential_fuel_items[r_name].made_by[recipe.name] = {
            result = result,
            ingredients = i_list
          }
BioInd.debugging.writeDebug("Added data for recipe %s to item %s: %s",
                  {r, r_name, potential_fuel_items[r_name].made_by})
        else
BioInd.debugging.writeDebug("Ignoring recipe %s", {r})
        end
      else
BioInd.debugging.writeDebug("No.")
      end
    end
  end
end
...
BioInd.debugging.writeDebug("Removing item %s from recipes", {remove_item})
  for item_name, item_data in pairs(tab) do
    ...
    for recipe_name, recipe_data in pairs(item_data.made_by or {}) do
      ...
      if not next(recipe_data.ingredients) then
        ...
        BioInd.debugging.writeDebug("Removed recipe %s from data of %s \"%s\"", {recipe_name, item_data.type, item_name})
      end
    end
  end
...
BioInd.debugging.show("Number of fuel_items", table_size(fuel_items))
  ...
  repeat
    run_again = false
    passes = passes + 1
BioInd.debugging.writeDebug("Pruning list (pass %s)", {passes})

    for item_name, item_data in pairs(tab) do
      -- No recipes are listed for that item
      if not (item_data.made_by and next(item_data.made_by)) then
        BioInd.debugging.writeDebug("Removing %s \"%s\" from list", {item_data.type, item_name})
        -- Remove item from potential_fuel_items
        tab[item_name] = nil
        BioInd.debugging.writeDebug("Removing %s \"%s\" from ingredients of potential fuel items",
...
BioInd.debugging.show("Number of remaining items", table_size(tab))
  until not run_again
BioInd.debugging.writeDebug("Finished pruning list of potential_fuel_items (needed %s passes)", {passes})
...
------------------------------------------------------------------------------------
BioInd.debugging.writeDebug("Pruning list of potential_fuel_items (pass %s)", {passes})
...
BioInd.debugging.writeDebug("Moved %s items to fuel_items", {passes})
BioInd.debugging.show("potential_fuel_items", potential_fuel_items)
BioInd.debugging.show("Number of potential_fuel_items", table_size(potential_fuel_items))
BioInd.debugging.show("fuel_items", fuel_items)
BioInd.debugging.show("Number of fuel_items", table_size(fuel_items))
BioInd.debugging.writeDebug("Moved %s items", {passes})
...
------------------------------------------------------------------------------------
BioInd.debugging.writeDebug("Removing obsolete recipes from list!")
passes = 0
repeat
  new_pass = false
  passes = passes + 1
  ...
  for item_name, item_data in pairs(fuel_items) do
    ...
    if fuel_value then
BioInd.debugging.writeDebug("%s \"%s\" already has a fuel_value. Moving it from fuel_items to fuel_values!",
                  {item_data.type, item_name})
      fuel_values[item_name] = {fuel_value = fuel_value, type = item_data.type}
      fuel_items[item_name] = nil
      new_pass = true
BioInd.debugging.writeDebug("Need another pass!")
      break
...
BioInd.debugging.writeDebug("Only one recipe for %s \"%s\"!", {item_data.type, item_name})
        item_data.recipe_order[1] = next(item_data.made_by)
--~ BioInd.debugging.show("item_data.recipe_order", item_data.recipe_order)

      -- If there are more, we sort them (least to most amount_per_sec)
      elseif n_ingredients > 1 then
BioInd.debugging.writeDebug("Ordering %s recipes for %s \"%s\"!", {n_ingredients, item_data.type, item_name})
        check = {}
...
--~ BioInd.debugging.show("check", check)

        -- Sort the array
        table.sort(check, function(a, b) return a and b and a.amount_per_sec < b.amount_per_sec end)
--~ BioInd.debugging.show("check after table.sort", check)

        -- Add sort order to item data
        for index, recipe in pairs(check) do
--~ BioInd.debugging.writeDebug("index: %s\trecipe: %s", {index, recipe}, "line")
          item_data.recipe_order[index] = recipe.name
        end
BioInd.debugging.show("item_data.recipe_order", item_data.recipe_order)
      -- This should never be reached!
      else
BioInd.debugging.writeDebug("Empty recipe list!")
        error("Empty recipe list!")
      end

    end
  end
BioInd.debugging.show("End of pass", passes)
until not new_pass
BioInd.debugging.show("fuel_items", fuel_items)
BioInd.debugging.show("fuel_values", fuel_values)
...
BioInd.debugging.writeDebug("Checking ingredient \"%s\"", {ingredient_name})
    -- Can't calculate fuel_value if we don't know that of all ingredients!
    if fuel_values[ingredient_name] then
BioInd.debugging.show("ingredient_data", ingredient_data)
BioInd.debugging.show("recipe_data.result", recipe_data.result)
      amount = ingredient_data.amount / (recipe_data.result.amount or 1)
BioInd.debugging.show("amount", amount)
BioInd.debugging.writeDebug("fuel_values[%s]: %s", {ingredient_name, fuel_values[ingredient_name].fuel_value or "nil"})
      fuel_value = fuel_value + amount * util.parse_energy(fuel_values[ingredient_name].fuel_value)
BioInd.debugging.show("fuel_value", format_fuel_value(fuel_value) .. "J")
    else
BioInd.debugging.show("Ingredient has no fuel_value yet", ingredient_name)
      check = false
      break
    end
  end
...
BioInd.debugging.entered_function()
BioInd.debugging.writeDebug("Trying to get fuel_value for fuel_items")
  ...
  for item_name, item_data in pairs(fuel_items) do
    ...
    fuel_items[item_name] = nil
BioInd.debugging.writeDebug("Removed %s \"%s\" from fuel_items (already stored in fuel_values)",
                  {item_data.type, item_name})
    -- We need to calculate the fuel_value
    else
BioInd.debugging.writeDebug("Checking %s \"%s\"", {item_data.type, item_name})
      fuel_value = data.raw[item_data.type][item_name].fuel_value
      -- We've already got a predefined fuel_value -- let's keep it!
      if fuel_value then
        fuel_values[item_name] = {fuel_value = fuel_value, type = item_data.type}
        fuel_items[item_name] = nil
BioInd.debugging.writeDebug("%s \"%s\" already has a fuel_value. Moved it from fuel_items to fuel_values!",
                    {item_data.type, item_name})
      -- Check if all ingredients already have a known fuel_value
      else
BioInd.debugging.writeDebug("fuel_items[%s]: %s", {item_name, fuel_items[item_name]})
...
BioInd.debugging.writeDebug("Checking recipe \"%s\"", {recipe_name})
        recipe_data = recipe_name and fuel_items[item_name].made_by[recipe_name]
BioInd.debugging.writeDebug("Recipe data: %s", {recipe_data or "nil"})

        -- Recipe exists
        if recipe_data then
          -- Try to calculate the fuel value
          fuel_value = calculate_fuel_value(recipe_data)
--~ BioInd.debugging.show("fuel_value", fuel_value)
          -- Missing ingredients, proceed with other items but schedule re-run!
          if fuel_value then
            fuel_values[item_name] = {fuel_value = fuel_value, type = item_data.type}
--~ BioInd.debugging.writeDebug("Added fuel_value for %s \"%s\" to list (%s)!",
                    --~ {item_data.type, item_name, fuel_values[item_name]})
            fuel_items[item_name] = nil
BioInd.debugging.writeDebug("'Moved recipe data from fuel_items to fuel_values", {item_name})
          else
BioInd.debugging.writeDebug("No fuel_value for %s \"%s\" yet", {item_data.type, item_name})
            run_again = true
          end
        -- Something has gone wrong: Remove recipe from item_data.recipe_order and
        -- schedule re-run to try the next recipe!
        else
BioInd.debugging.writeDebug("Removing \"%s\" from recipe_order.", {recipe_name})
          table.remove(item_data.recipe_order, 1)
          run_again = true
BioInd.debugging.writeDebug("Next entry: \"%s\"", {item_data.recipe_order[1]})
        end
      end
    end
  end
BioInd.debugging.show("Need to run again", run_again and "yes" or "no" )
BioInd.debugging.entered_function("leave")
  return run_again
end
...
BioInd.debugging.entered_function()
BioInd.debugging.writeDebug("conflict_item: %s\ttry_next: %s", {conflict_item, try_next or "nil"})
...
BioInd.debugging.writeDebug("Loop detected: Already checked item %s!", {try_next})
BioInd.debugging.entered_function("leave")
    return try_next
...
BioInd.debugging.writeDebug("Checking recipe \"%s\": %s", {recipe_name, recipe_data})
    if recipe_data.ingredients then
--~ BioInd.debugging.writeDebug("Recipe has ingredients")
      -- Return immediately if the recipe requires conflict_item
      if recipe_data.ingredients[conflict_item] then
BioInd.debugging.writeDebug("Conflicting item \"%s\" is used in recipe \"%s\" to make %s \"%s\"",
                  {conflict_item, recipe_name, fuel_items[try_next] and fuel_items[try_next].type, try_next})
        return try_next
      end

--~ BioInd.debugging.writeDebug("Need to check ingredients of recipe")
      -- Otherwise check all ingredients if *they* require conflict_item
      for ingredient_name, ingredient_data in pairs(recipe_data.ingredients) do
--~ BioInd.debugging.show("Checking ingredient", ingredient_name)
        result = find_circle(conflict_item, ingredient_name)
BioInd.debugging.writeDebug("Checked ingredients of \"%s\" for conflict_item \"%s\". Result: \"%s\"",
                  {ingredient_name, conflict_item, result or "nil"})
        if result then
--~ BioInd.debugging.show("result", result)
          return result
        else
BioInd.debugging.writeDebug("Checking next ingredient")
        end
      end
    end
  end
...
BioInd.debugging.writeDebug("Need to get fuel_value for %s fuel_items (pass %s)", {table_size(fuel_items), passes})
  new_pass = get_fuel_values()
--~ BioInd.debugging.show("Another pass has been requested", new_pass)
--~ BioInd.debugging.writeDebug("fuel_items from previous run: %s\tfuel_items now: %s", {old, table_size(fuel_items)})

  -- Try to remove deadlocks
  if new_pass and old == table_size(fuel_items) then
BioInd.debugging.writeDebug("Impossible situation: Deadlock detected!")
    -- Deadlock is caused by an ingredient in a recipe making fuel_items
    for item_name, item_data in pairs(fuel_items) do
BioInd.debugging.show("Checking item", item_name)
...
BioInd.debugging.show("Checking recipe", recipe_name)
...
BioInd.debugging.show("i_name", i_name)

              -- Obvious conflict: item is used as ingredient for itself!
              if i_name == item_name then
  BioInd.debugging.writeDebug("Removing ingredient %s from recipe \"%s\" -- makes %s \"%s\"!",
  ...
  BioInd.debugging.writeDebug("Checking if ingredient \"%s\" is used recursively", i_name)
                conflict_item = find_circle(item_name, i_name)
  BioInd.debugging.show("conflict_item", conflict_item)
              --~ else
--~ BioInd.debugging.writeDebug("Skipping ingredient \"%s\" (already has a fuel_value)", {i_name})
              end

            -- No ingredients left. Break out of the loop!
            else
BioInd.debugging.writeDebug("Recipe \"%s\" has no ingredients left -- removing recipe!", {recipe_name})
              item_data.made_by[recipe_name] = nil
BioInd.debugging.show("item_data.made_by", item_data.made_by)
...
BioInd.debugging.writeDebug("conflict_item: %s\ti_name: %s\trecipe_removed: %s",
...
BioInd.debugging.show("fuel_items[" .. conflict_item .. "]", fuel_items[conflict_item])
BioInd.debugging.writeDebug("Removing conflicting item %s from fuel_items and recipes", {conflict_item})
...
BioInd.debugging.writeDebug("Check if last recipe was removed from fuel_items[%s]", {item_name})
          remove_no_recipes(fuel_items)
          break
        end
      end
...
BioInd.debugging.show("new_pass", new_pass)
BioInd.debugging.show("old == table_size(fuel_items)", old == table_size(fuel_items))
...
BioInd.debugging.writeDebug("Calculated fuel_value for all fuel_items (needed %s passes)", {passes})
BioInd.debugging.show("fuel_items", fuel_items)
BioInd.debugging.show("Number of fuel_items", table_size(fuel_items))
BioInd.debugging.show("fuel_values", fuel_values)
BioInd.debugging.show("Number of entries in fuel_values", table_size(fuel_values))
BioInd.debugging.writeDebug("Needed %s passes", {passes})
...
--~ BioInd.debugging.writeDebug("Set fuel_value of %s \"%s\" to %s", {item_data.type, item_name, item.fuel_value})
