...
local function add_hidden_entity(tab, base, hidden_entity)
  ...
  -- Bio cannon
  if tab == "Bio_Cannon_Table" then
    optional = {delay = 0}

  -- Arboretum
  elseif tab == "Arboretum_Table" and hidden_entity == "radar" then
...
BioInd.show("size of global[tab]", table_size(global[tab]))
  ...
  ------------------------------------------------------------------------------------
  -- Arboretum
  if tab == "Arboretum_Table" and hidden_entity == "radar" then
...
BioInd.show("force.players", force.players)
BioInd.writeDebug("Force: %s\tNumber of players: %s\tPlayers: %s", {force_name, table_size(force.players), force.players}, "line")
